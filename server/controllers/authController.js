...
export const registerRoute = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: registeredData,
      msg: "User resisterd successfully",
    });
  } catch {
    res
      .status(400)
      .json({ success: true, error: error, msg: "Something went wrong" });
  }
};
...
export const postContentRoute = async (req, res) => {
  const token = req.headers["x-access-token"]; // const header = { x-access-token: "jwt_generated_token" }
  try {
    const decoded = jwt.verify(token, process.env.KEY); // { "name": "Naresh", "email": "naresh@gmail.com", "iat": 1707895336 }
    const email = decoded.email;
